package io.github.trethore.graphene.neoforge.internal.platform;
...
import io.github.trethore.graphene.neoforge.internal.screen.GrapheneScreenBridge;
import io.github.trethore.graphene.neoforge.internal.util.MinecraftReferences;
  ...
  private static GrapheneLifecycle lifecycle() {
    return new GrapheneLifecycle() {
      ...
      public void onStarted(Runnable action) {
        NeoForge.EVENT_BUS.addListener(ClientStartedEvent.class, event -> action.run());
      }

      @Override
      public void onStopping(Runnable action) {
        NeoForge.EVENT_BUS.addListener(ClientStoppingEvent.class, event -> action.run());
      }
    };
  }
    ...
    return new GrapheneModResolver() {
      ...
      public String resolveModId(Class<?> anchorClass) {
        ...
        for (ModContainer modContainer : ModList.get().getSortedMods()) {
          if (!containsClass(modContainer, classFilePath)) {
            continue;
          }
          String candidateModId = modContainer.getModId();
        ...
        if (resolvedModId == null) {
          throw new IllegalArgumentException("No mod contains " + anchorClass.getName());
        }
      ...
      private boolean containsClass(ModContainer modContainer, String classFilePath) {
        IModFileInfo owningFile = modContainer.getModInfo().getOwningFile();
        return owningFile != null && owningFile.getFile().getContents().containsFile(classFilePath);
      }
    };
  }
